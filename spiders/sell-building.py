from krishascrapy.items import SellBuilding
from .spider import BuildingSpider

class SellBuildingSpider(BuildingSpider):
    name = "sell-building"
    list_url_template = 'https://krisha.kz/prodazha/zdanija/?page='
    
    def __init__(self):
        self.ad = SellBuilding()
    
    def parse_ad(self, response):
        ad = super().parse_ad(response)
        ad['exchange'] = self.getFieldValue(response, 'has_change')
        
        mortgage = response.css('.offer__parameters-mortgaged')
        if len(mortgage):
            ad['mortgage'] = 1
        else:
            ad['mortgage'] = 0
        
        return ad
